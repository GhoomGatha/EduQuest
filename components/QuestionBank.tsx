...
const QuestionBank: React.FC<QuestionBankProps> = ({ questions, onAddQuestion, onEditQuestion, onDeleteQuestion, lang, showToast }) => {
  ...
  const filteredQuestions = useMemo(() => {
    return questions.filter(q => {
      const classFilterMatch = (filterClass === '' || q.class === filterClass);
      const marksFilterMatch = (filterMarks === '' || q.marks === filterMarks);
      const difficultyFilterMatch = (filterDifficulty === '' || q.difficulty === filterDifficulty);
      const chapterFilterMatch = (filterChapter === '' || q.chapter === filterChapter);
      const tagFilterMatch = (filterTag === '' || (q.tags && q.tags.some(tag => tag.toLowerCase().includes(filterTag.toLowerCase()))));
      
      let searchMatch = true;
      if (searchTerm) {
        if (filterChapter) {
          // If a chapter is selected, search is "local" to just the question text.
          searchMatch = q.text.toLowerCase().includes(searchTerm.toLowerCase());
        } else {
          // If no chapter is selected, search is "global" across text and chapter names.
          searchMatch = q.text.toLowerCase().includes(searchTerm.toLowerCase()) || 
                        q.chapter.toLowerCase().includes(searchTerm.toLowerCase());
        }
      }
      
      return classFilterMatch && marksFilterMatch && difficultyFilterMatch && chapterFilterMatch && tagFilterMatch && searchMatch;
  ...
  const searchPlaceholder = useMemo(() => {
    if (filterChapter) {
      return t('searchWithin', lang).replace('{chapter}', filterChapter);
    }
    ...
    <div className="p-2 sm:p-4 space-y-4">
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
        <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold font-serif-display text-slate-800">{t('bank', lang)}</h2>
            <button
